...
export class NotificationController {
    ...
    async deleteNotification(@Param('id') id: string, @Req() req: { user: JwtDto }) {
        try {
            console.log('📥 [BACKEND] Delete notification request:', id);
            const result = await this.notificationService.deleteNotification(id, req.user);
            console.log('✅ [BACKEND] Notification deleted');
            return ApiResponse.success(result);
        } catch (error) {
            console.error('❌ [BACKEND] Error deleting notification:', error);
            throw error;
        }
    }
    ...
    @Delete('read/all')
    @ApiOkResponse({
        description: 'Eliminar todas las notificaciones leídas',
        type:  ApiResponse<any>,
    })
    async deleteAllRead(@Req() req: { user: JwtDto }) {
        try {
            console.log('📥 [BACKEND] Delete all read notifications request from user:', req.user.sub);
            const result = await this. notificationService.deleteAllRead(req.user);
            console. log('✅ [BACKEND] All read notifications deleted');
            return ApiResponse.success(result);
        } catch (error) {
            console.error('❌ [BACKEND] Error deleting read notifications:', error);
            throw error;
        }
    }
}